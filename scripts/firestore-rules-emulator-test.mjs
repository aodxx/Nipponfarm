...
try {
  ...
  // Existing owner can update while preserving ownership.
  ...
  // Another active staff user cannot steal ownership by changing userId to their own UID.
  ...
  // Admin retains the intentional override path.
  await assertSucceeds(updateDoc(doc(adminDb, 'bills', 'bill-a'), { vendorName: 'Admin correction' }));

  // PENDING/RESIGNED accounts cannot directly use payroll owner paths anymore.
  ...
  // Verify the owner record still exists after allowed/denied update attempts.
