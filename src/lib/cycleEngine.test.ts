...
test('parity increments at FARROW and remains unchanged at WEAN', () => {
  ...
  assert.deepEqual(calculateNextSowState('PREGNANT', 2, 'FARROW'), { status: 'LACTATING', parity: 3 });
  assert.deepEqual(calculateNextSowState('LACTATING', 3, 'WEAN'), { status: 'RECOVERY', parity: 3 });
