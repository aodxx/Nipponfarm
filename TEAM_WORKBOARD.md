...
| `NP-RULES-02` | A | Static Firebase rules scenario matrix | `NIPON-SEC-01` | `NIPON-FB-01` | `security/rules-scenario-matrix-2026-09-04` | `IN_PROGRESS` | `docs/verification-packet/evidence/rules-scenario-matrix.md` เท่านั้น | — | มี matrix สำหรับ unauthenticated, ADMIN, STAFF, PENDING, RESIGNED, wrong-owner; ไม่แก้ `firestore.rules` หรือ `storage.rules`; มี evidence schema |
...
| `NP-WF-03` | B | Five core workflow verification matrix | `NIPON-QA-01` | `NIPON-LEAD-01` | `test/core-workflow-matrix-2026-09-04` | `IN_PROGRESS` | `docs/CORE_WORKFLOW_VERIFICATION_MATRIX.md` และ `docs/verification-packet/evidence/core-workflow-matrix.md` | — | ระบุ entry point, collections/services, roles, happy/error path, permission, reload/relogin, mobile usability และ blocker ของ Sow, Receipt, Pig Sale, Payroll, Maintenance |
