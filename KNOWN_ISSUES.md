...
| P1 | Advance duplicate guard ยังไม่ atomic | `employeeService.addAdvance` query รายการเดิมก่อน `addDoc`; ครอบคลุม retry ปกติและมี tests แต่ยังมี race ได้เมื่อ submit พร้อมกัน | อาจเกิดคำขอเบิกซ้ำใน concurrent write; เป็นความเสี่ยงธุรกรรมการเงิน | เพิ่ม idempotency key หรือ transaction-safe write พร้อม integration test ก่อน production-ready |
| P1 | Payroll ยังไม่มี audit trail ครบ | approval/change flow ยังไม่มีหลักฐาน actor, previous/new status และเวลาใน audit record | ตรวจสอบย้อนหลังและ reconciliation ได้ไม่ครบ | ออกแบบ audit schema และทดสอบก่อนแก้ rules หรือ migration |
